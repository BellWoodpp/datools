...
export async function generateMetadata({ params }: LocaleToolsPageProps) {
  const resolvedParams = await params;
  const locale = resolvedParams.locale;
  const normalizedLocale = locales.find((l) => l === locale);
  const title = "Data Analysis Tools Directory - DaTools";
  const description =
    "Browse curated data analysis tools by category. Find BI, product analytics, ETL/ELT, warehouses, notebooks, governance, and AI assistants with quick links to each detail page.";
  ...
  const localizedTitle = dictionary.homeFeed?.heading ?? "Data Analysis Tools Directory";
  return {
    title: `${localizedTitle} - DaTools`,
    description,
    alternates: {
      canonical: buildCanonicalPath(normalizedLocale, "data-analysis-tools"),
    },
  };
}

export default async function LocaleDataAnalysisToolsPage({ params }: LocaleToolsPageProps) {
  const resolvedParams = await params;
  const locale = resolvedParams.locale;
  const normalizedLocale = locales.find((l) => l === locale);
            ...
            >
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {linkedTools.map((tool) => {
                  const normalizedLink = tool.link
                    ? tool.link.startsWith("/")
                      ? tool.link
                      : `/${tool.link}`
                    : "#";
                  const detailHref =
                    normalizedLocale === "en" ? normalizedLink : `${basePath}${normalizedLink}`;
                  return (
                    <Link
                      key={tool.name}
                      href={detailHref}
