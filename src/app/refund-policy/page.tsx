...
export const metadata: Metadata = {
  title: `${copy[defaultLocale as CopyLocale]?.title ?? copy.en.title} - DaTools`,
  description:
    copy[defaultLocale as CopyLocale]?.subtitle ?? copy.en.subtitle,
  alternates: {
    canonical: buildCanonicalPath(undefined, "refund-policy"),
  },
};

export default function RefundPolicyPage() {
  const locale = (defaultLocale as CopyLocale) in copy ? (defaultLocale as CopyLocale) : "en";
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <hr />
        <p className="text-sm">
          <Link href={defaultLocale === "en" ? "/terms" : `/${defaultLocale}/terms`}>
            {content.links.terms}
          </Link>
        </p>
      </div>
